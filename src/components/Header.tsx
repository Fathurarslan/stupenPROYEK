            ...
            n.children ? (
              <li key={n.label} className="relative max-[860px]:w-full">
                <button
                  ...
                  className="flex w-full cursor-pointer items-center gap-1 rounded-md border-0 bg-transparent px-3 py-2 text-[14px] font-medium text-inherit hover:bg-white/12 max-[860px]:justify-between max-[860px]:opacity-70"
                  onClick={() => setDropdownTerbuka(dropdownTerbuka === n.label ? null : n.label)}
                >
                  {n.label}
                  <span aria-hidden="true">{dropdownTerbuka === n.label}</span>
                </button>
                <ul
                  className={`absolute top-full left-0 z-30 m-0 min-w-[180px] list-none flex-col rounded-md bg-white p-1 text-tinta shadow-lg max-[860px]:static max-[860px]:min-w-0 max-[860px]:rounded-none max-[860px]:bg-transparent max-[860px]:p-0 max-[860px]:text-white max-[860px]:shadow-none ${
                ...
                >
                  {n.children.map((c) => (
                    <li key={c.path}>
                      <Link
                        className="block rounded-md px-3 py-2 text-[14px] font-medium text-inherit no-underline hover:bg-sawah/10 max-[860px]:hover:bg-white/12"
                        to={c.path}
                        onClick={() => {
                          setBuka(false);
                          setDropdownTerbuka(null);
